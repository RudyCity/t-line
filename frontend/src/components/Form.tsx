...
export const Input: React.FC<InputProps> = ({ className = '', ...props }) => {
  return (
    <input
      className={`form-input w-full bg-black/40 border border-white/5 rounded-lg px-3 py-2 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/25 transition-all duration-200 ${className}`}
      {...props}
    />
  );
};
...
export const Select: React.FC<SelectProps> = ({ className = '', options, ...props }) => {
  return (
    <select
      className={`form-input w-full bg-black/40 border border-white/5 rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/25 transition-all duration-200 cursor-pointer ${className}`}
      {...props}
    >
      {options.map((opt) => (
...
export const Button: React.FC<ButtonProps> = ({
  ...
  if (variant === 'primary') {
    variantClass = 'bg-purple-600 hover:bg-purple-500 text-white shadow-lg shadow-purple-600/10 hover:shadow-purple-500/30';
  } else if (variant === 'secondary') {
    variantClass = 'bg-slate-800/40 hover:bg-slate-800/80 text-slate-200 border border-white/5 hover:border-white/10';
